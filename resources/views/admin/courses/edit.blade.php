    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="/admin/courses/{{ $course->id }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ $course->title }}" required class="w-full px-4 py-2 border rounded-lg">
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">الوصف</label>
                <textarea name="description" required rows="4" class="w-full px-4 py-2 border rounded-lg">{{ $course->description }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="thumbnail" value="{{ $course->thumbnail }}" class="w-full px-4 py-2 border rounded-lg">
            </div>
...
<script>
let roadmapCount = 0;
const existingRoadmap = @json($course->roadmap ?? []);

function addRoadmapLevel(levelData = null) {
    ...
    const levelHtml = `
        <div class="roadmap-level flex items-center gap-3 bg-gray-50 p-4 rounded-lg">
            <div class="flex-1">
                ...
                <input type="text" name="roadmap_levels[]" placeholder="أدخل اسم المستوى" value="${levelTitle}" class="w-full px-3 py-2 border rounded-lg text-sm">
            </div>
...
// Initialize with existing roadmap or one empty level
document.addEventListener('DOMContentLoaded', function() {
    if (existingRoadmap && existingRoadmap.length > 0) {
        existingRoadmap.forEach(level => addRoadmapLevel(level));
    } else {
        addRoadmapLevel();
    }
});
